	...
	if ((level.roundLimit > 1 || (!level.roundLimit && level.scoreLimit != 1)) && !level.forcedEnd)
	{
		...
		if (isDefined(game["PROMOD_KNIFEROUND"]) && game["PROMOD_KNIFEROUND"])
		{
			...
			[
				...
				for (i = 0; i < level.players.size; i++)
					if (level.players[i].pers["team"] == "spectator") level.players[i] setClientDvars("shout_scores_attack", game["teamScores"][game["attackers"]], "shout_scores_defence", game["teamScores"][game["defenders"]]);
			}
...
updatePlacement()
{
	...
	for (i = 1; i < placementAll.size; i++)
	{
		...
		for (j = i - 1; j >= 0 && (playerScore > placementAll[j].score || (playerScore == placementAll[j].score && player.deaths < placementAll[j].deaths)); j--) placementAll[j + 1] = placementAll[j];
...
disableBombsites()
{
	if (level.gametype == "sd" && isDefined(level.bombZones))
		for (j = 0; j < level.bombZones.size; j++) level.bombZones[j] maps\mp\gametypes\_gameobjects::disableObject();
}
...
Callback_StartGameType()
{
	...
	for (i = 0; i < stringNames.size; i++)
		if (!isstring(game["strings"][stringNames[i]])) precacheString(game["strings"][stringNames[i]]);
...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration)
{
	...
	if (isDefined(level.scorebot) && level.scorebot && !level.rdyup) game["promod_scorebot_ticker_buffer"] += "kill" + lpattackname + "" + scWeapon + "" + self.name + "" + sHeadshot;
